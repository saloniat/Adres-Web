...
$(function() {
    ...
    $(".chat-sidebar").scroll(function(e){
        ...
        if(leftMsgProcessing)
            return false;
        console.log($(".chat-sidebar").scrollTop())
...
function loadConversation(messageId, lastMessageId, showLoader=false) {

    $.ajax({
        url: '/admin/chat/chat-history/',
        type: 'post',
        dataType: 'json',
        cache: false,
...
function active_chatroom(element){
    ...
    loadConversation(roomName, '', true);
}
...
function save_chat_message(){
    ...
    if(!$('#chat_msg_form').valid()){
        ...
        $.ajax({
            url: '/admin/chat/save-chat-message/',
            type: 'post',
            dataType: 'json',
            cache: false,
