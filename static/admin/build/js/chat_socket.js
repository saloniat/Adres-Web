...
active_chatroom =  (element) => {
    ...
    var proeprtySubdomainName = $(element).attr('data-property-subdomain');

    var masterChatPropId = $(element).attr('data-property');
    var masterChatPropName = $(element).attr('data-property-name');
    if(masterChatPropId){
        var propertySubdomainLink = window.location.protocol + '//' + proeprtySubdomainName + '.' + subdomain_url
        var chat_name = masterChatName+'<br><span><a href="'+ propertySubdomainLink +'asset-details/?property_id='+masterChatPropId+'" target="_blank">'+masterChatPropName+'</a><span>';
...
genrateChatRoomHtml = (data) => {
    var unreadCounter = (data.unread_msg_cnt > 0)? 'unread':''
        ...
        userImage = (data.doc_file_name) ? azure_blob_url + data.bucket_name + '/' + data.doc_file_name : '/static/admin/images/no-image.jpg';
        userTypeText = (data.seller_id == data.sender_id)? ((data.seller_site_id && data.seller_site_id == data.domain_id)? 'Broker': 'Agent'): 'Buyer'
...
genrateConversationHtml = (data) => {
    ...
    var userImage = (data.image_name) ? azure_blob_url + data.bucket_name + '/' + data.image_name : '/static/admin/images/no-image.jpg';
