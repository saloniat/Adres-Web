...
$("#set_auto_bids").on("click", function(){
    var auto_bid_amount = parseInt($("#set_auto_bids").attr("data-auto-bid-amount"));
    $("#auto_bid_amount").val(auto_bid_amount);
    $('#set_auto_bid_modal').modal('show');
});

$("#auto_bid_cancel").on("click", function(){
    $('#set_auto_bid_modal').modal('hide');
    $("#auto_bid_amount_error").hide();
    $("#success_message").hide();
});

$(document).on('keyup','#auto_bid_amount',function(){
    var auto_bid_amount = $(this).val();
    if(isNaN(auto_bid_amount) || auto_bid_amount == "" || auto_bid_amount < 1){
        $("#auto_bid_amount_error").text("Please enter valid amount");
        $("#auto_bid_amount_error").show();
        return false;
    }else{
        $("#auto_bid_amount_error").hide();
    }
...
$(document).on('click','#auto_bid_submit',function(){
    var auto_bid_amount = parseInt($("#auto_bid_amount").val());
    if(isNaN(auto_bid_amount) || auto_bid_amount == "" || auto_bid_amount < 1){
        return false;
    }
    var bid_increments_value = bid_increments.replace(/,/g, "");
    console.log(bid_increments_value)
    socket.emit("setAutoBid", {"property_id": property_id, "auction_id": auction_id, "domain_id": site_id, "min_bid_amount": start_bid, "auto_bid_amount": auto_bid_amount, "bid_increment": bid_increments_value, "user_id": session_user_id,"ip_address": ip_address, "registration_id": registration_id});
    socket.on('setAutoBid',function(data) {
        console.log("=============set_auto_bid==============");
        console.log(data);
        if(data.error != 1 && (data.status == 200 || data.status == 201)){
            $("#success_message").addClass("success").html(data.msg).show();
            socket.emit("checkBid", {"user_id":session_user_id, "property_id": property_id, "auction_id": auction_id, "domain_id": site_id});
        }
        else{
            $("#success_message").addClass("error").html(data.msg).show();
        }
    });
});

$("#confirm_auto_bid_true").on("click", function(){
    if($("#autoBidNowFrm").valid()){
        $('#submitAutoBid').prop('disabled',true);
        $('#confirm_auto_bid_true').prop('disabled',true);
        var auto_bid_amt = $('#auto_bid_amt').val();
        auto_bid_amt = auto_bid_amt.toString().replace(/,/g, '').replace('$', '').replace('€', '').replace('&euro;', '');
        bid_increments = bid_increments.toString().replace(/,/g, '').replace('€', '').replace('&euro;', '');
        $('#confirmAutoBidModal').modal('hide');
         ...
         socket.emit("setAutoBid", {"property_id": property_id, "auction_id": auction_id, "domain_id": site_id, "min_bid_amount": start_bid, "auto_bid_amount": parseInt(auto_bid_amt), "bid_increment": parseInt(bid_increments), "user_id": session_user_id,"ip_address": ip_address, "registration_id": registration_id});
         socket.on('setAutoBid',function(data) {
          //if((data.status == 200 || data.status == 201) && data.error == 0){
            if(data.error == 0){
                 $("#auto_bid_msg").html(data.msg);
                 $("#auto_bid_msg").addClass("text-success");
                 $("#autoBidModal").modal("show");
                 socket.emit("checkBid", {"user_id":session_user_id, "property_id": property_id, "auction_id": auction_id, "domain_id": site_id});
            }else{
                 $("#auto_bid_msg").html(data.msg);
                 $("#auto_bid_msg").addClass("text-danger");
                $('#autoBidModal').modal('show');
            }
            //$('#submitAutoBid').removeProp('disabled');
            //$('#confirm_auto_bid_true').removeProp('disabled');
            $('#submitAutoBid').removeAttr('disabled');
            $('#confirm_auto_bid_true').removeAttr('disabled');
      });
    }
  });
    ...
    if(typeof(data.data) != 'undefined' && data.data && (res_user_id == log_user_id || (res_user_id == '' && log_user_id == ''))){
        try{
           ...
           if(typeof(data.data.start_date) != 'undefined'){
            ...
            var new_start_date = getCstDate(data.data.start_date, 'm j, Y','');
           ...
           if(typeof(data.data.end_date) != 'undefined'){
            ...
            var new_end_date = getCstDate(data.data.end_date, 'm j, Y','');
            ...
            // set auto bid
            if(data.data.auto_bid && parseInt(data.data.auto_bid_setup) == 2 && is_registered == 'True' && registration_approved == 'True' && today >= start_date_time && today < end_date_time){
                //auto_bid_amount = numberFormat(parseInt(data.data.user_auto_bid_amount));
                $(".set_auto_bid_section").show();
                $("#set_auto_bids").attr("data-auto-bid-amount", data.data.user_auto_bid_amount);
                //-----New
                if(data.data.user_auto_bid_amount > 0){
                    auto_bid_amount = numberFormat(parseInt(data.data.user_auto_bid_amount));
                }else{
                    auto_bid_amount = 0;
                }
                $("#auto_bid_section").show();
                $("#auto_bid_amt").val("$"+auto_bid_amount);
            }else{
                $(".set_auto_bid_section").hide();
                //----New
        ...
        $('#submitUserBid').removeAttr('disabled');
        $('#confirm_bid_true').removeAttr('disabled');
    }
  });
